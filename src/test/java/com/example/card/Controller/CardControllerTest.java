...
class CardControllerTest {
  ...
  void getAllCards_returnsOk() throws Exception {
    when(cardService.getAllCards())
        .thenReturn(List.of(buildCard("Card A", "Travel", "Bank A", 4.5)));
  ...
  void getCardById_whenFound_returnsOk() throws Exception {
    when(cardService.getCardById(1L))
        .thenReturn(Optional.of(buildCard("Card A", "Travel", "Bank A", 4.5)));
  ...
  void getCardById_whenMissing_returnsNotFound() throws Exception {
    ...
    mockMvc.perform(get("/card/1")).andExpect(status().isNotFound());
  }
  ...
  void getTop4RatingCards_whenEmpty_returnsNotFound() throws Exception {
    ...
    mockMvc.perform(get("/card/top4-rating-cards")).andExpect(status().isNotFound());
  }
